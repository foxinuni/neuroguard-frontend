...
export default function RegisterPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!deviceId.trim()) { setError("Ingresa el ID del dispositivo"); return; }
    ...
    try {
      ...
      setError(map[err.code] || err.message || "Error al registrarse");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      // If user has no profile yet, ProtectedRoute will redirect to /setup
      navigate("/dashboard", { replace: true });
    } catch (err) {
    ...
    <div className="flex min-h-screen">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-gradient-to-br from-brand-800 via-brand-700 to-brand-900 items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-12">
          ...
          <p className="mt-3 text-lg text-brand-300">Crea tu cuenta y comienza a monitorear tu salud</p>
      ...
      {/* Right panel — form */}
      <div className="flex w-full items-center justify-center px-6 py-8 lg:w-1/2 bg-brand-900 light:bg-white">
        <div className="w-full max-w-md">
          ...
          <div className="mb-5 lg:mb-6">
            <h2 className="text-2xl font-bold text-white light:text-gray-900">Crear cuenta</h2>
            <p className="mt-1 text-sm text-brand-300 light:text-gray-500">Registra tus datos para acceder al sistema de monitoreo</p>
          </div>
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-3 rounded-xl border border-brand-700/50 bg-brand-800/60 px-4 py-3 text-sm font-medium text-white
              hover:bg-brand-800 hover:shadow-sm active:scale-[0.98]
          ...
          <div className="my-5 flex items-center gap-3">
            ...
            <span className="text-xs text-brand-400/70 light:text-gray-400">o regístrate con email</span>
            <div className="h-px flex-1 bg-brand-700/60 light:bg-surface-border" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-brand-200 light:text-gray-700">Nombre completo</label>
              <input
                type="text"
                required
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full rounded-xl border border-brand-700/50 bg-brand-800/60 px-4 py-2.5 text-sm text-white
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-brand-200 light:text-gray-700">Confirmar</label>
                <input
                  type="password"
                  required
                  value={confirmPwd}
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-brand-200 light:text-gray-700">ID del dispositivo</label>
              <input
                type="text"
                required
                value={deviceId}
              ...
              />
              <p className="mt-1 text-xs text-brand-400/60 light:text-gray-400">
                Se encuentra en la etiqueta de tu dispositivo NeuroGuard
              </p>
            </div>
          ...
          <p className="mt-6 text-center text-sm text-brand-300 light:text-gray-500">
            ¿Ya tienes cuenta?{" "}
            <Link to="/login" className="font-semibold text-brand-400 light:text-brand-600 hover:text-brand-300 light:hover:text-brand-700 transition-colors">
              Inicia sesión
            </Link>
          </p>
